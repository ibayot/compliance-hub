...
| `reviewer` | Legacy backward-compatible compliance access; retained for prior-version compatibility |
| `compliance_officer` | Primary compliance role; reviews/approves documents, manages KPI/MOV/issuance data |
...
As a `compliance_officer`, `reviewer`, `auditor`, or `super_admin`, I can call `POST /issuances` so that a regulatory reference is formally entered.
...
As a `focal`, `technician`, `compliance_officer`, `reviewer`, `auditor`, or `super_admin`, I can call `GET /issuances` with optional filters (`authority`, `category`, `search`, `is_active`) so that relevant issuances are found.
...
As a `compliance_officer`, `reviewer`, `auditor`, or `super_admin`, I can call `PUT /issuances/:id` so that issuance fields are corrected or enriched.
...
As a `compliance_officer`, `reviewer`, `auditor`, or `super_admin`, I can call `POST /issuances/:id/documents/:documentId` so that compliance evidence is associated with the regulatory requirement.
...
As a `compliance_officer`, `reviewer`, `auditor`, or `super_admin`, I can call `POST /issuances/:id/attachment` to upload a PDF or file blob so that the source issuance document is stored.
...
As a `compliance_officer`, I can verify the status values of each metric are one of `compliant`, `warning`, `non_compliant`, or `unknown` so that the status domain is enforced.
...
As a `compliance_officer`, viewing the KPI dashboard for a period with no monitoring entries shows zero values or an empty state, not a crash.
...
As a `compliance_officer`, submitting review remarks containing Unicode characters, quotes, or HTML entities does not corrupt the stored data.
...
As a `compliance_officer`, creating a MOV artifact with `quarter: null` (annual scope) is accepted.
...
As a `compliance_officer`, attempting to return a document without providing `remarks` is blocked.
...
| ISS-001 | Positive | Create issuance as compliance_officer | Issuance created | | | | | | |
