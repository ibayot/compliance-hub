...
> **Release `v0.0.83` (2026-06-09):** Fixed a bug in the Detailed Ratings view where the Support Type filter was ignored by the backend API. Also fixed the horizontal scrolling bleed issue in the Role Capabilities Matrix Settings page by safely encapsulating the overflow area within an inner `TableContainer` component.
...
> **Release `v0.0.60` (2026-06-02):** SLA indicators like `isOverdue` and `isNearingSLA` introduced. New detailed API endpoints for Ratings created.
> **Release `v1.3.0.9` (2026-02-27):** KPI Dashboard refinements: the Unit KPI Scores table now always shows every unit available to you — even units with partial data (e.g. Finance in Q3 with only July and August entries) — and displays `—` in the Score column when the period isn't complete yet. The Trend sparkline now reflects the last month that has data, and always starts from zero (no previous December anchor). The Unit KPI Scores chart and table now occupy the full width of the page. The Unit Detail section is now stacked directly below the Scores panel, only appears when you click a unit row, and can be closed with the × button in its header. The Band Distribution pie now shows a white/transparent dashed slice for units that have data but are not yet fully scored for the period.
> **Release `v1.3.0.8` (2026-02-27):** KPI Dashboard improvements: the Overall Score card progress bar now turns green/amber/red based on the band thresholds. Chart legends are removed — the Color column in each table already serves this purpose. The Band Distribution pie no longer shows hover text. When you select a specific unit in the Unit Filter, the multi-unit Scores chart hides and the Unit Detail expands to fill the full width automatically. Finance Unit data now draws correctly in Q3, Semester 2, and Annual views — lines appear up to the last month with data and leave a gap for months with no entries. Dark/Light mode can now be toggled directly from the user account menu (top-right) without going to Settings. The Units page Reportorial Document Types section now loads automatically when you expand a unit accordion — no more "None Yet" on first open.
> **Release `v1.3.0.7` (2026-02-27):** Stability fix — the backend server now starts reliably without crashing with a database connection error. In the KPI Dashboard, all unit and KPI trend lines are now always visible in the chart regardless of which frequency filter is active — even if a unit has no data for certain months, the other periods still show the line with a gap. Stale seed files with old KPI codes removed; only the correct seed file remains.
