...
> **Release `v1.4.0` (2026-03-03):** Issuances coverage is expanded further across applicable laws, IRRs, standards, Executive Orders, DICT/NPC circular references, and NCSP plan context. Issuances now include amendment metadata (`is_amendment`, `amended_issuance_number`, `ict_amendment_notes`) and a dedicated `ICT Related Amendments` table column for traceable ICT-impacting legal updates.
> **Patch (`v1.4.0`, 2026-03-03):** Issuances filters for Authority/Category are now persistent checkbox multi-select dropdowns (selected values stay visible while filtering). Manual upload-first issuance assessment is enabled via `issuance-file-drop/` + `scripts/classify_issuance_drop.py`, and `ICT-ISSUANCE-RELEVANCE-MAP.md` now includes complete applicable list plus explicit exclusion/deferred reasons.
> **Patch 2 (`v1.4.0`, 2026-03-03):** Deep-dive reassessment now parses all dropped PDFs and generates `issuance-file-drop/deepdive-assessment.md` + enriched `classification-results.csv` (page count, topic hints, external context). All `AO`/`MC` files are included as `INTERNAL_POLICY` categories and are no longer auto-marked for removal.
> **Patch 3 (`v1.4.0`, 2026-03-03):** Public-service relevance is now an explicit inclusion criterion for reassessment. `RA-9485` is included (required for anti-red-tape/public service delivery governance), while `DICT Department Circular HRA-003 s2025` remains deferred removal as telecom-provider specific. Latest deep-dive counts: `41 included`, `10 deferred removal`, `0 manual review`.
> **Patch 4 (`v1.4.0`, 2026-03-03):** Internal AO/MC issuances are now seeded directly in Issuances (`issuance-041..050`) and DPO-related operational coverage is explicitly seeded via `NPC-CIRCULAR-17-01`. Issuance category labels in the UI now render in human-readable title format (capitalized, underscores removed in display).
> **Patch 5 (`v1.4.0`, 2026-03-03):** Issuances now support per-issuance file attachments stored in the database (`attachment_blob` + metadata), with secure upload/view/download endpoints. Table actions are consolidated into an ellipsis menu to reduce action overload, and client-side pagination is enabled to avoid long-scroll behavior. Title click behavior now resolves as: open source link when present; otherwise open attached file when available.
> **Release `v1.3.0.22` (2026-03-03):** Issuances filters now use dropdown options for **Authority**, **Category**, and **Status** (All/Active/Inactive). Issuance category filtering is backed by `issuance_type`, and each issuance can now be set active/inactive from row action or Add/Edit form (for superseded/inactive policies). Seeded `applicability_scope` and `relevance_notes` narratives are expanded for deeper implementation guidance.
> **Release `v1.3.0.21` (2026-03-03):** Issuances module now includes the full ICT-relevant baseline across laws, circulars, memorandums, IRRs, and international standards (ISO/NIST) for operations, governance, cybersecurity, business continuity, disaster response, safety, and applicable use. Issuances table structure is aligned to `Issuance Number | Title | Authority | Issue Date | Status | Mapped Documents | Actions`, with a new action button that opens an Applicability/Relevance modal.
> **Release `v1.3.0.20` (2026-03-03):** Issuances module baseline is now curated to ICT-relevant operational/governance/safety issuances only, with authoritative source links (primarily Official Gazette) and a dedicated traceability guide at `ICT-ISSUANCE-RELEVANCE-MAP.md`. Seed entries removed standards-only and generic placeholder references to keep module scope enforceable and ICT-focused.
> **Release `v1.3.0.19` (2026-03-03):** KPI/Reports trend refinement — removed forced arrowhead tilt so arrowheads follow the true angle of change, and enabled multi-point zigzag sparkline rendering for Quarterly/Semestral/Annual trend cells to reflect actual period movement. Existing data baseline and repository functionality remain unchanged.
> **Release `v1.3.0.18` (2026-03-03):** Final UX/data refinement pass — KPI/Reports trend arrowheads now use a subtle minimum tilt (~5°) for small deltas (still directional, less awkward). Metrics applicability is tightened so unit-targeted sets remain isolated to the intended unit document types while Jan/Feb/Mar 2025 pending queue samples (`doc-017..022`) are seeded as `Monthly Report`. Repository modal now includes a direct Download button so users can download the latest original file without leaving preview.
> **Release `v1.3.0.17` (2026-03-03):** KPI/Reports UI refinement + seed reliability fix — Direction indicators now show arrows only (`↑` / `↓`), and trend sparkline arrowheads are intentionally tilted more for small deltas so direction remains visible. Backend seed command is repaired with `backend/src/database/seed.ts`; running `npm run db:seed` now reliably loads all 12 metrics (3 per type), unit-targeted applicability mappings, and pending Documents queue samples (`doc-017..022`) with focal ownership for visibility.
> **Release `v1.3.0.16` (2026-03-04):** KPI/Reports UX + scoring update — `lower_is_better` now uses `target/actual` scoring behavior (clamped), KPI and Reports trend sparklines now render arrowheads, and single-unit KPI tables now show per-KPI direction (`↑ Higher` / `↓ Lower`). KPI `KPIs by Performance Band` pie now respects selected filters and marks incomplete-period KPI carryover as transparent dashed `PARTIAL`. Repository table removes status/compliance columns (records are already compliant-ready) while preserving View and Download actions for report reuse. Seed adds Jan/Feb/Mar 2025 report samples for IT and Finance units.
> **Release `v1.3.0.15` (2026-03-04):** KPI/Reports alignment patch — Monthly trend range reverted to previous-month→selected-month (Jan keeps 0-start), while Quarterly/Semestral/Annual trend direction now uses average-based comparison (historical average vs current period). Reports KPI legends removed. Documents and Reviews now operate as pending-only queues, and Repository serves ready/compliant outputs with inline modal preview (no route redirect). Seed expanded with additional issuance references and extra pending 2026 per-unit sample documents; User Manual and release docs updated accordingly.
> **Release `v1.3.0.9` (2026-02-27):** KPI Dashboard layout overhaul — Unit KPI Scores panel is now full-width and its table always shows ALL available units (including partial-period units like Finance in Q3), with `—` for score/KPI count when data is incomplete; Trend sparkline uses the last available data point and always starts from 0 (no Dec previous-year anchor); Unit Detail panel is now stacked below the Scores panel (not side-by-side), shown only when a unit is clicked, and has a close (×) button; Band Distribution pie includes a transparent/dashed slice for partial-period units.
> **Release `v1.3.0.8` (2026-02-27):** KPI UI polish — score card progress bar now uses band color (green/amber/red); chart legends removed (redundant with Color column); pie hover tooltip removed; Finance Unit lines now draw in Q3/S2/Annual views (partial data fix — fetches timeseries for all visible units regardless of period data availability); unit filter auto-hides the Unit KPI Scores panel and expands Unit Detail to full width; theme Dark/Light mode toggle added to AppBar user menu; Units accordion now auto-loads Reportorial Document Types on expand.
