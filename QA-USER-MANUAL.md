...
> **Release `v0.0.81` (Backend) / `v0.0.72` (Frontend) (QA Implementation, 2026-06-09):** Verify: (1) **Create User Text Helpers** — `helperText` has been removed from all input fields in Create New User and Edit User Profile. (2) **Autofill Disabled** — `autoComplete="off"` is enforced on user forms, and `autoComplete="new-password"` on password fields. (3) **Support Type Metrics** — the "Tickets by Support Type" pie chart/metrics in Ticket Reports now evaluate *all* total tickets to display volume distribution by support type, regardless of resolution status. (4) **Role Capabilities Matrix UI** — the `Role` sticky column correctly overlays content by using explicit hex `bgcolor: '#ffffff'` and `zIndex`, preventing switches/text from showing through the background on horizontal scroll.
...
| #  | Test Step                                                                              | Expected Result                                         |
|----|----------------------------------------------------------------------------------------|---------------------------------------------------------|
| 1  | Load KPI page as `fo2admin@dswd.gov.ph`                                                | No NaN SQL errors in backend; dashboard loads           |
