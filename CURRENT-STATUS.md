...
> Update (`v0.0.14` — 2026-04-14, `microservices` branch): **QA findings batch implemented for DB reference cleanup and route ownership correction.** (1) Replaced runtime/config/script `rictms_compliance` defaults with `compliance_hub` in active backend runtime files and reset scripts. (2) Added split DB env keys in backend env configs for users/ticketing/compliance routing. (3) Moved attendance API ownership to users service by exposing attendance controller via users service and removing attendance controller exposure from ticketing service. (4) Updated gateway `/api/attendance` proxy target to users service. (5) Decoupled auth module from static tickets-module import to prevent users service route contamination. (6) Added users-service query fallback guards for partial DB migration states to avoid hard-fail login paths when join artifacts are missing. (7) **Patch-only versioning** — bumped `0.0.13` -> `0.0.14` (x/y unchanged). **Validation:** backend build ✅, frontend build ✅, backend tests ✅ (4/4), attendance routing ownership check ✅ (`ticketing /api/attendance` returns 404), integration accessibility ⚠️ partial due local DB missing split migration objects and missing MySQL CLI in this environment.
> Update (`v0.0.13` — 2026-04-14, `microservices` branch): **QA findings batch completed for inactivity re-authentication and deployment/documentation deliverables.** (1) Implemented 15-minute inactivity lock in frontend auth session flow with activity listeners and lock-state enforcement. (2) Added unlock path requiring password re-entry for local-auth users. (3) Added backend `POST /auth/reauthenticate` endpoint and auth-service password validation path for session unlock checks. (4) Extended `/auth/me` profile payload with `authProvider` to support provider-specific lock handling. (5) Added `deployment.md` with repository-based step-by-step split-container deployment instructions. (6) Added `INHOUSE-QA-USER-STORIES.md` and `MAIN-SYSTEM-DOCUMENTATION.md` to satisfy QA documentation findings. (7) **Patch-only versioning** — bumped `0.0.12` -> `0.0.13` (x/y unchanged). **Validation:** backend build ✅, frontend build ✅, backend tests ✅ (4/4), smoke test script ⚠️ failed in local runtime with login `500` response.
> Update (`v0.0.12` — 2026-04-14, `microservices` branch): **QA ownership findings closed for duplicate split-DB tables and cross-db access compatibility.** (1) Enforced single-table ownership so `users` exists as a base table only in `compliance_hub_users`, `units` only in `compliance_hub`, and `attendance` only in `compliance_hub_users`. (2) Replaced cross-service duplicates with compatibility views in ticketing/compliance/users schemas to preserve existing runtime queries and APIs. (3) Hardened `backend/database/microservices-migrate.sql` for repeatable execution with base-table checks, self-copy guards, and view-aware cleanup. (4) Updated ticketing startup migration to ensure attendance ownership in users DB and regenerate ticketing views (`users`, `units`, `attendance`). (5) Added fallback cross-db API endpoint `GET /api/users/federated` for user retrieval through federated DB objects. (6) Preserved microservice-unavailability output behavior in gateway/frontend (no regression). (7) **Patch-only versioning** — bumped `0.0.11` -> `0.0.12` (x/y unchanged). **Validation:** backend build ✅, frontend build ✅, backend tests ✅ (4/4), local DB ownership verification ✅.
> Update (`v0.0.11` — 2026-04-14, `microservices` branch): **QA follow-up completed for split DB naming, attendance table rename, and microservice availability UX.** (1) Renamed service DB targets to `compliance_hub_users`, `compliance_hub_ticketing`, and `compliance_hub` in compose and DB scripts. (2) Renamed ticketing attendance storage from `tech_attendance` to `attendance` in entity mapping, runtime startup migration, query paths, and UAT reset scripts. (3) Hardened `backend/database/microservices-migrate.sql` source selection so migration uses a schema with actual legacy data, including fallback support for both `ricms_compliance` and `rictms_compliance`. (4) Added gateway `503` service-unavailable responses with explicit messaging and per-service health flags. (5) Added frontend service-availability hook plus compliance-route unavailable fallback and sidebar service-aware visibility filtering. (6) **Local migration execution completed:** verified schemas `compliance_hub`, `compliance_hub_ticketing`, `compliance_hub_users` exist on localhost and verified `attendance` exists in ticketing schema. (7) **Patch-only versioning** — bumped `0.0.10` -> `0.0.11` (x/y unchanged). **Validation:** backend build ✅, frontend build ✅, backend tests ✅ (4/4).
> Update (`v0.0.10` — 2026-04-14, `microservices` branch): **QA findings closed for attendance legacy-role removal and role-definition-based attendance grouping.** (1) Removed attendance usage of `FOCAL`, `TECHNICIAN`, `TECHNICIAN_DESKTOP`, `TECHNICIAN_IT_SUPPORT`, `TECHNICIAN_IT_STAFF`, and `TECHNICIAN_DESKTOP_STAFF` from attendance service/controller/frontend role gates. (2) Implemented centralized attendance role grouping (`desktop_support`, `it_support`, `pantawid_ict_support`, `ito`, `all`) in `attendance.service.ts`. (3) Attendance role selection now resolves from `role_definitions` (`assignable=true`) while excluding `user`, `super_admin`, and the removed legacy roles. (4) Cleaned custom-role attendance branch logic by replacing it with centralized group-based filtering. (5) **Patch-only versioning** — bumped `0.0.9` -> `0.0.10` (x/y unchanged). **Validation:** backend build ✅, frontend build ✅, backend tests ✅ (4/4), smoke suite ✅.
