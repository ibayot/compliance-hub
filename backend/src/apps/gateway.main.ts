...
const SERVICE_DOMAINS: Record<string, string[]> = {
  ...
  ticketing: ['/api/tickets', '/api/attendance', '/api/ticket-settings', '/api/knowledge-base', '/api/notifications'],
  compliance: [
    '/api/documents',
    '/api/document-types',
    '/api/comparisons',
...
async function bootstrap() {
  ...
  const registerServiceRoutes = (prefix: '/api' | '/api/v1') => {
    ...
    app.use(
      ...
      createServiceProxy(`${ticketingServiceUrl}/api/notifications`, 'ticketing'),
    );
    app.use(
      `${prefix}/attendance`,
