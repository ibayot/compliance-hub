...
export class DutyService {
  ...
  async getStaffOptions(actor: Actor) {
    this.assertAdmin(actor);
    const users = await this.usersClient.getUsers();
    return users
      .filter((user) => user.active !== false)
      .filter((user) => user.role !== UserRole.SUPER_ADMIN && user.role !== UserRole.USER)
      .filter((user) => this.roleCaps.isTechnician(user.role))
      .map((user) => ({
        id: user.id,
        email: user.email,
        ...
        technicianEligible: true,
        autoAssignmentEligible: user.autoAssignmentEligible,
      }));
  }
