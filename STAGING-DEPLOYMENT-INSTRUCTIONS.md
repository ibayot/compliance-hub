...
2. The DBA must create the following **four** blank databases with `utf8mb4` encoding:
...
## 4. Persistent File Storage (Ticket Attachments & Escalation Proofs)

Comment images and escalation proof photos are saved to disk inside the ticketing service container.
Without a volume mount, these files are **permanently deleted** every time the container is rebuilt.

To ensure uploads survive across deployments, the `ticketing-service` section of `docker-compose.staging.yml` **must** include:

### Required: volume mount + environment variables

```yaml
ticketing-service:
  ...
  environment:
    - COMMENT_ATTACHMENT_STORAGE_ROOT=/app/uploads/comments
    - ESCALATION_STORAGE_ROOT=/app/uploads/escalations
    # ... all other existing env vars
  volumes:
    - ticketing_uploads:/app/uploads
```

And at the bottom of the file, in the top-level `volumes:` block:

```yaml
volumes:
  # ... other volumes (db_data, redis_data, etc.)
  ticketing_uploads:
```

> **Why this works:** Docker named volumes (`ticketing_uploads`) are stored on the host filesystem and are NOT removed when containers are rebuilt with `--build`. Files written to `/app/uploads/` inside the container are persisted to this volume automatically.

> **One-time migration note:** If there are existing attachment files inside a running container, copy them to the volume before rebuilding:
> ```bash
> docker exec rictms_ticketing_service tar czf - /app/uploads | docker run --rm -v ticketing_uploads:/restore -i alpine tar xzf - -C /
> ```
