...
function parsePlanItems(entry: MovArtifact): string[] {
  const metadataItems = Array.isArray(entry.metadata_json?.items)
    ? (entry.metadata_json?.items as string[])
    : [];
...
export default function MovBuilderPage() {
  ...
  const [lastReportKind, setLastReportKind] = useState<
    'register' | 'monitoring' | 'assessment' | ''
  >('');
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      enqueueSnackbar(error?.response?.data?.message || 'Failed to load MoV Builder data.', {
        variant: 'error',
      });
    }
  ...
  useSse(['GLOBAL_SETTINGS_UPDATED'], loadData);
  ...
  const generateRegisterReport = async (registerType: RegisterType) => {
    try {
      ...
      enqueueSnackbar(error?.response?.data?.message || 'Failed to generate register report.', {
        variant: 'error',
      });
    }
  };
  ...
  const generateMonitoringMatrix = async () => {
    try {
      ...
      enqueueSnackbar(error?.response?.data?.message || 'Failed to generate monitoring matrix.', {
        variant: 'error',
      });
    }
  };
  ...
  const generateAssessmentReport = async () => {
    try {
      ...
      enqueueSnackbar(error?.response?.data?.message || 'Failed to generate assessment report.', {
        variant: 'error',
      });
    }
  };
  ...
  const buildPrintHtml = (): string => {
    ...
    const parseFooterInput = (
      rawValue: string,
    ): { startPage: number; bodyHtml: string; hasAny: boolean } => {
    ...
    if (headerImage1)
      headerParts.push(
        `<img src="${headerImage1}" style="height:39px;width:auto;max-height:39px;object-fit:contain;display:inline-block;vertical-align:middle;" alt="Header 1" />`,
      );
    if (headerImage2)
      headerParts.push(
        `<img src="${headerImage2}" style="height:45px;width:auto;max-height:45px;object-fit:contain;display:inline-block;vertical-align:middle;" alt="Header 2" />`,
      );
    ...
    const counterResetStyle = subOffset !== 0 ? `html { counter-reset: page ${subOffset}; }` : '';
    ...
    const firstCssContent = buildCssContent(
      diffFirstFooter ? firstFooterConfig : subsequentFooterConfig,
    );

    const firstPageOverrideCss =
      diffFirstFooter && firstFooterConfig.hasAny
        ? `@page :first { @bottom-center { content: ${firstCssContent}; line-height: 1.0; overflow: hidden; } }`
        : '';
    const suppressFooterCss =
      !subsequentFooterConfig.hasAny && !(diffFirstFooter && firstFooterConfig.hasAny)
    ...
    const buildSignatureCell = (label: string, name: string, pos: string, des: string): string => {
      ...
      return [
        ...
        name
          ? `<p style="margin:4px 0 0 0;font-weight:bold;">${escapeHtml(name.toUpperCase())}</p>`
...
<body>
  ...
  const addScheduleEntry = async () => {
    ...
    try {
      ...
      enqueueSnackbar(error?.response?.data?.message || 'Failed to add schedule entry.', {
        variant: 'error',
      });
    }
  };
  ...
  const updateScheduleEntry = async (
    ...
    try {
      ...
      enqueueSnackbar(error?.response?.data?.message || 'Failed to update schedule entry.', {
        variant: 'error',
      });
    }
  };
  ...
  const savePlanEdit = async (entry: MovArtifact) => {
    ...
    if (!editingPlanTitle.trim() || items.length === 0) {
      enqueueSnackbar('Plan title and at least one bullet item are required.', {
        variant: 'warning',
      });
      return;
    }
    ...
    try {
      ...
      enqueueSnackbar(error?.response?.data?.message || 'Failed to update assessment plan.', {
        variant: 'error',
      });
    }
  };
  ...
  const addPlanEntry = async () => {
    ...
    try {
      const index =
        Math.max(1, ...planByYear.map((item) => Number(item.metadata_json?.year_index || 0) || 0)) +
        1;
      await movApi.create({
      ...
      enqueueSnackbar(error?.response?.data?.message || 'Failed to add assessment plan.', {
        variant: 'error',
      });
    }
  };
  ...
  const deletePlanEntry = async (entry: MovArtifact) => {
    try {
      ...
      enqueueSnackbar(error?.response?.data?.message || 'Failed to delete assessment plan entry.', {
        variant: 'error',
      });
    }
  };
  ...
  const saveArtifactStatus = async (artifact: MovArtifact) => {
    try {
      ...
      enqueueSnackbar(error?.response?.data?.message || 'Failed to update artifact status.', {
        variant: 'error',
      });
    }
  };
  ...
  const handleSavePreset = async () => {
    ...
    const payload = {
      header1_data: headerImage1,
      header2_data: headerImage2,
      page_footer: pageFooter,
      diff_first_footer: diffFirstFooter,
      first_page_footer: firstPageFooter,
    ...
    try {
      ...
      if (existing) {
        ...
        await movApi.create({
          artifact_type: 'print_settings',
          title: presetName.trim(),
          scope: 'settings',
          period_year: new Date().getFullYear(),
          content_markdown: 'print_settings',
  ...
  const handleLoadPreset = () => {
    ...
    if (!preset?.metadata_json) {
      enqueueSnackbar('Select a preset first.', { variant: 'warning' });
      return;
    }
    ...
    if (m.prepared_by_position !== undefined)
      setPreparedByPosition(m.prepared_by_position as string);
    if (m.prepared_by_designation !== undefined)
      setPreparedByDesignation(m.prepared_by_designation as string);
    if (m.approved_by_name !== undefined) setApprovedByName(m.approved_by_name as string);
    if (m.approved_by_position !== undefined)
      setApprovedByPosition(m.approved_by_position as string);
    if (m.approved_by_designation !== undefined)
      setApprovedByDesignation(m.approved_by_designation as string);
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 2 }}>
        Organized workspace for register reporting, assessment reporting, plan updates, and schedule
        monitoring.
      </Typography>
              ...
              <CardContent>
                <Stack spacing={1.5}>
                  ...
                  <Button
                    variant="contained"
                    size="small"
                    onClick={() => generateRegisterReport('legal')}
                  >
                  ...
                  <Button
                    variant="contained"
                    size="small"
                    onClick={() => generateRegisterReport('standards')}
                  >
                  ...
                  <Stack spacing={1.5}>
                    {kpiGapRows.length === 0 && (
                      <Typography variant="body2" color="text.secondary">
                        No KPI gaps detected for this period. You may still add manual remarks
                        below.
                      </Typography>
                    )}
                    {kpiGapRows.map((row) => (
              ...
              <AccordionDetails>
                <Stack spacing={2}>
                  ...
                  <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    {headerImage1 && (
                      <>
                        ...
                        />
                        <IconButton size="small" onClick={() => setHeaderImage1('')}>
                  ...
                  <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    {headerImage2 && (
                      <>
                        ...
                        />
                        <IconButton size="small" onClick={() => setHeaderImage2('')}>
                          <CloseIcon fontSize="small" />
                        </IconButton>
                      </>
                    )}
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        checked={diffFirstFooter}
                        onChange={(e) => setDiffFirstFooter(e.target.checked)}
                      />
                    }
                  ...
                  <Grid container spacing={1}>
                    ...
                    <Grid item xs={12} md={4}>
                      <TextField
                        ...
                        fullWidth
                        helperText="Abbreviated, e.g. ITO I"
                      />
                    </Grid>
                    <Grid item xs={12} md={4}>
                      <TextField
                  ...
                  <Grid container spacing={1}>
                    ...
                    <Grid item xs={12} md={4}>
                      <TextField
                        ...
                        fullWidth
                        helperText="Abbreviated, e.g. ITO II"
                      />
                    </Grid>
                    <Grid item xs={12} md={4}>
                      <TextField
              ...
              <CardContent>
                {planByYear.length === 0 ? (
                  <Typography color="text.secondary">
                    No plan years found. Add one below.
                  </Typography>
                ) : (
                          ...
                          >
                            ...
                            >
                              <CardContent>
                                ...
                                >
                                  <Chip
                                    label={String(entry.period_year)}
                                    size="small"
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={4}>
                    <TextField
                      ...
                      fullWidth
                      placeholder="e.g. Year 1 – Context & Governance"
                    />
                  </Grid>
                  ...
                  <Grid item xs={12}>
                    <Button variant="contained" startIcon={<AddIcon />} onClick={addPlanEntry}>
                      Add Plan Year
                    </Button>
                  </Grid>
                </Grid>
      ...
      {tab === 2 && (
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <Card>
              <CardHeader
                title={`Assessment Schedule – ${year} Q${quarter}`}
                subheader="Update status and remarks per activity."
                action={
                  <Button
                    size="small"
                    variant="outlined"
                ...
                <Grid container spacing={2} sx={{ mb: 2 }}>
                  ...
                  <Grid item xs={12} md={2}>
                    <TextField
                      ...
                      label="Owner"
                      value={scheduleOwner}
                      onChange={(e) => setScheduleOwner(e.target.value)}
                    />
                  </Grid>
                  <Grid item xs={12} md={2}>
                    <TextField
                  ...
                  <Grid item xs={12} md={1}>
                    <Button fullWidth variant="contained" onClick={addScheduleEntry}>
                      Add
                    </Button>
                  </Grid>
                </Grid>
                  ...
                  <TableBody>
                    {scheduleEntries.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={6}>
                          <Typography variant="body2" color="text.secondary">
                            No schedule entries for this quarter.
                          </Typography>
                        </TableCell>
                      </TableRow>
                    )}
              ...
              <TableBody>
                {allArtifacts.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={6}>
                      <Typography variant="body2" color="text.secondary">
                        No artifacts saved for this period.
                      </Typography>
                    </TableCell>
                  </TableRow>
                )}
                      ...
                      {editingArtifactId === artifact.id ? (
                        <Stack direction="row" spacing={0.5}>
                          <IconButton
                            size="small"
                            color="primary"
                            onClick={() => saveArtifactStatus(artifact)}
                          >
                        ...
                        <IconButton
                          ...
                          onClick={() => {
                            ...
                            setEditingArtifactStatus(artifact.status);
                          }}
                        >
