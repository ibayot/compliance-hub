...
export default function UserManualPage() {
  ...
  const hasCapabilityAccess = (capKey: string): boolean => {
    ...
    if (myCap && myCap[capKey as keyof typeof myCap]) {
      return true;
    }
    ...
    <Box>
      <Box mb={4}>
        ...
        <Typography variant="body1" color="text.secondary">
          Welcome to the application user manual. The topics below are curated specifically for your role and capabilities.
        </Typography>
      </Box>
            ...
            <DialogContent dividers>
              <Box>
                ...
                <List dense disablePadding>
                  {selectedManual.details.activities.map((input) => (
                    <ListItem key={input.name} disableGutters alignItems="flex-start" sx={{ mb: 1 }}>
                      <ListItemText
                        primary={
                ...
                <List dense disablePadding>
                  {selectedManual.details.results.map((output) => (
                    <ListItem
                      key={output.name}
                      disableGutters
