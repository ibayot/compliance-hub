...
export default function DutiesPage() {
  ...
  return <Box>
    ...
    <Dialog open={!!dialog} onClose={()=>setDialog(null)} fullWidth><DialogTitle>{dialog === 'roster' ? 'Manage Shared Duty Roster' : `${form.id ? 'Edit' : 'Add'} ${dialog}`}</DialogTitle><DialogContent><Stack spacing={2} mt={1}>
      ...
      {dialog==='exception' && <><DateField label="Date" value={form.exceptionDate} change={(v:string)=>setForm({...form,exceptionDate:v})}/><UserField users={users.filter(isDutyStaff)} value={form.userId} change={(v:number)=>setForm({...form,userId:v})}/><SelectField label="Type" value={form.type} values={['travel_order','exam','assistance','pacd','canvass','due_to_ta','other']} change={(v:string)=>setForm({...form,type:v})}/><TextField label="Remarks" multiline value={form.remarks||''} onChange={(e)=>setForm({...form,remarks:e.target.value})}/></>}
...
function UserField({users,value,change}:any){return <FormControl><InputLabel>Name</InputLabel><Select label="Name" value={value||''} onChange={(e)=>change(+e.target.value)}>{users.filter((u:any)=>u.active!==false&&u.role!=='user').map((u:UserRecord)=><MenuItem key={u.id} value={u.id}>{userName(users,u.id)}</MenuItem>)}</Select></FormControl>}
