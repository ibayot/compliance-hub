...
export default function DutiesPage() {
  ...
  return <Box>
    ...
    <Dialog open={!!dialog} onClose={()=>setDialog(null)} fullWidth><DialogTitle>{dialog === 'roster' ? 'Manage Duty Roster' : dialog === 'relievers' ? `Immediate Relievers — ${form.venueType || ''}` : `${form.id ? 'Edit' : 'Add'} ${dialog}`}</DialogTitle><DialogContent><Stack spacing={2} mt={1}>
      ...
      {dialog==='roster' && <><FormControl><InputLabel>Members in rotation order</InputLabel><Select multiple label="Members in rotation order" value={form.userIds||[]} onChange={(e)=>{const value=e.target.value; const userIds=(Array.isArray(value) ? value : String(value).split(',')).map(Number).filter((id:number)=>Number.isInteger(id)); setForm({...form,userIds,odOnlyUserIds:(form.odOnlyUserIds||[]).filter((id:number)=>userIds.includes(id))});}}>{users.filter(isDutyStaff).map((u)=><MenuItem key={u.id} value={u.id}>{userName(users,u.id)}</MenuItem>)}</Select></FormControl><FormControlLabel control={<Switch checked={!!form.odOverrideEnabled} onChange={(e)=>setForm({...form,odOverrideEnabled:e.target.checked})} />} label="Enable OD Override" />{form.odOverrideEnabled && <><Alert severity="info">OD-only members will rotate only for Officer of the Day. Other roster members will rotate for ROC, Conference, and OpCen.</Alert><FormControl><InputLabel>OD-only members</InputLabel><Select multiple label="OD-only members" value={form.odOnlyUserIds||[]} onChange={(e)=>{const value=e.target.value; setForm({...form,odOnlyUserIds:(Array.isArray(value) ? value : String(value).split(',')).map(Number).filter((id:number)=>Number.isInteger(id))});}}>{users.filter((u:any)=>(form.userIds||[]).includes(u.id)).map((u)=><MenuItem key={u.id} value={u.id}>{userName(users,u.id)}</MenuItem>)}</Select></FormControl></>}</>}
      {dialog==='relievers' && <><Alert severity="warning">These technicians replace the normal {form.venueType} assignment for {form.meetingDate}. Select one or more active technicians from the shared roster.</Alert><FormControl><InputLabel>Immediate relievers</InputLabel><Select multiple label="Immediate relievers" value={form.userIds||[]} onChange={(e)=>{const value=e.target.value; setForm({...form,userIds:(Array.isArray(value) ? value : String(value).split(',')).map(Number).filter((id:number)=>Number.isInteger(id))});}}>{data.roster.filter((row:any)=>!data.rosterConfig.odOverrideEnabled || !row.odOnly).map((row:any)=><MenuItem key={row.userId} value={row.userId}>{row.name}</MenuItem>)}</Select></FormControl><TextField required label="Reason" multiline minRows={3} value={form.reason||''} onChange={(e)=>setForm({...form,reason:e.target.value})} helperText="Required. Explain why management requested this replacement." />{(form.userIds||[]).length > 0 && <Button color="error" variant="outlined" onClick={async()=>{try{await dutiesApi.clearMeetingRelievers(form.id);setDialog(null);setForm({});await load(true);enqueueSnackbar('Immediate relievers removed; normal rotation restored.',{variant:'success'});}catch(e:any){enqueueSnackbar(e?.response?.data?.message||'Unable to remove immediate relievers.',{variant:'error'});}}}>Restore Normal Rotation</Button>}</>}
