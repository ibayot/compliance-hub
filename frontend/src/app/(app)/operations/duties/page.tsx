...
export default function DutiesPage() {
  ...
  const removeReservation = async (id: string) => { try { await dutiesApi.deleteReservation(id); enqueueSnackbar('Meeting schedule deleted.', { variant: 'success' }); await load(true); } catch (e: any) { enqueueSnackbar(e?.response?.data?.message || 'Unable to remove meeting.', { variant: 'error' }); } };
  const release = async () => {
  ...
  return <Box>
    ...
    <Dialog open={!!dialog} onClose={()=>setDialog(null)} fullWidth><DialogTitle>{dialog === 'roster' ? 'Manage Duty Roster' : `${form.id ? 'Edit' : 'Add'} ${dialog}`}</DialogTitle><DialogContent><Stack spacing={2} mt={1}>
