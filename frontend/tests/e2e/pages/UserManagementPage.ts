...
export class UserManagementPage {
  ...
  async verifyUserInTable(email: string) {
    ...
    for (let i = 0; i < 10; i++) {
      try {
        await expect(tableCell).toBeVisible({ timeout: 2000 });
        return; // found it
      } catch {
  ...
  async editUserRole(email: string, newRoleName: string) {
    ...
    await expect(this.page.getByText(/User profile updated successfully/i).first()).toBeVisible({
      timeout: 10000,
    });
  }
  ...
  async deactivateUser(email: string) {
    ...
    const activeSwitch = dialog.locator(
      'input[type="checkbox"][name="isActive"], input[type="checkbox"][name="active"]',
    );
