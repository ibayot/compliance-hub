...
test.describe('Suite 4 — TICKET LIFE CYCLE', () => {
  ...
  test('Round 1: Ticket Creation & Resolution', async ({ page }) => {
    ...
    const nonUserAccounts = [
      accounts.admin,
      accounts.sectionHead,
      accounts.itSupportJr,
      accounts.pantawidIct,
      accounts.devLead,
      accounts.desktopJr,
      accounts.desktopSr1,
      accounts.desktopSr2,
      accounts.complianceOfficer,
      accounts.cybersec,
    ];
    ...
    // Admin explicitly assigns tickets to avoid load balancer flakiness in dirty test environments
    // await loginPage.login(accounts.admin.email, accounts.admin.password);
    // await dashboardPage.navigateTo('Tickets');

    // Assign IT ticket to IT Support Jr
    // await ticketsPage.assignTicketToUserContaining(subjectIT, 'Godofredo');
    // await dashboardPage.navigateTo('Tickets');

    // Assign Desktop ticket to Desktop Support Jr
    // await ticketsPage.assignTicketToUserContaining(subjectDesktop, 'Jaymark');
    // await dashboardPage.navigateTo('Tickets');

    // Assign Pantawid ticket to Pantawid ICT
    // await ticketsPage.assignTicketToUserContaining(subjectPantawid, 'James Arnel');
  ...
  test('Out of Office Configuration & Round 2 Creation', async ({ page }) => {
    ...
    // Explicitly assign subDesktop2 to Desktop Jr (Jaymark) so he can escalate it in the next test
    // await ticketsPage.assignTicketToUserContaining(subDesktop2, 'Jaymark');
  ...
  test('Escalation & De-Escalation (Round 2)', async ({ page }) => {
    ...
    const row = page
      .locator('tbody tr')
      .filter({ hasText: /(Assigned|In Progress)/i })
      .filter({ hasText: 'Desktop Support' })
      .first();
