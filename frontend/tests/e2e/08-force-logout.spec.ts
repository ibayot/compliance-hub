...
test.describe('Force Logout on Role Change', () => {
  ...
  test('User is forcefully logged out when their role is changed by an admin', async ({ browser }) => {
    ...
    // Navigate to Settings -> User Management manually
    ...
    const isDesktopSupportJr = roleText.toLowerCase().includes('desktop') && roleText.toLowerCase().includes('jr');
    ...
    const originalRoleToRestore = isDesktopSupportJr ? 'Desktop Support Junior' : 'Compliance Officer';
    ...
    // Wait for the 60-second heartbeat to detect the role change
    // The test timeout was already increased to 120s to allow for this.
    
    // We wait up to 65 seconds for the snackbar to appear naturally.
    const snackbar = userPage.locator('text=Your role has been updated. Please log in again to apply changes.');
    await expect(snackbar).toBeVisible({ timeout: 70000 });

    // After 1.5 seconds, they should be logged out and redirected to login
    await expect(userPage).toHaveURL(/.*\/login\?reason=role_changed/, { timeout: 10000 });

    // 4. Restore the role (Clean up)
